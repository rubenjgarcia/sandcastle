/* globals describe, it */
/* eslint no-multi-str:0, handle-callback-err:0 */
...
describe('Pool', function () {
  ...
  it('should not be created if there are zero or negative amount of specified instances', function (finished) {
    try {
      var pool = new Pool({numberOfInstances: 0}) // eslint-disable-line
      equal(false, true)
    } catch (error) {
      notEqual(-1, error.message.indexOf('Can\'t create a pool with zero instances'))
      finished()
    }
  })
})
