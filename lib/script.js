...
Script.prototype.createClient = function (methodName, globals) {
  ...
  var connectClient = function (socket) {
    ...
    if (typeof socket === 'object' && socket.host) {
      client = net.createConnection(socket.port, socket.host, connectListener)
    } else {
      client = net.createConnection(socket, connectListener)
    }
  ...
  if (this.sandcastle) {
    this.sandcastle.sandboxReady(function () {
      connectClient(_this.sandcastle.getSocket())
    })
  } else {
    connectClient(this.socket)
  }
}
...
Script.prototype.onExit = function (methodName, data) {
  ...
  var parsed = _this._parseChunk(data)
